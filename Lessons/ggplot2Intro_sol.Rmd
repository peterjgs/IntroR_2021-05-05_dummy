...
# Import data to plot
We are going to plot some data about cars that is contained in the `cars93.csv` file. First we import the data.
...
3. Create a boxplot of `uptake` by `Treatment`, with different fills for each treatment.
...
There are 5 variables, two continuous, two factor and one ordinal factor.

2. Create a histogram of the variable `uptake`.

```{r}
p1<-ggplot(CO2,aes(x=uptake))+geom_histogram()
p1
```
...
```{r}
p2<-ggplot(CO2,aes(x=Treatment,y=uptake,fill=Treatment))+geom_boxplot()
p2
```

4. Create a scatter plot of `uptake` by `conc`, with different colours and symbols by Treatment.

To figure out how to change symbols, have a look in the help file. Use the code below or type `geom_point` into the search field in the Help tab in RStudio.

```{r eval=FALSE}
?geom_point
```

Looks like `shape` might be the right attribute to change, so I try that.

```{r}
p3<-ggplot(CO2,aes(x=conc,y=uptake,color=Treatment,shape=Treatment))+geom_point()
p3
```
